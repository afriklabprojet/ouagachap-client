...
class DeliveryTrackingInfo {
  ...
  factory DeliveryTrackingInfo.fromJson(Map<String, dynamic> json) {
    ...
    return DeliveryTrackingInfo(
      ...
      statusLabel:
          json['status_label'] ?? _getStatusLabel(json['status'] ?? 'pending'),
...
class RealTimeTrackingService {
  ...
  // Stream controllers (recréés à chaque session de tracking)
  final StreamController<DeliveryTrackingInfo> _trackingController =
      StreamController<DeliveryTrackingInfo>.broadcast();
  final StreamController<CourierPosition> _courierPositionController =
      StreamController<CourierPosition>.broadcast();

  // Streams publics
  Stream<DeliveryTrackingInfo> get trackingStream => _trackingController.stream;
  Stream<CourierPosition> get courierPositionStream =>
      _courierPositionController.stream;
  ...
  Future<void> _fetchTrackingInfo() async {
    ...
    try {
      final token = getIt<SecureTokenService>().token;

      final response = await http
          .get(
            Uri.parse(
              '${AppConstants.baseUrl}/orders/$_currentOrderId/tracking',
            ),
            headers: {
      ...
      if (response.statusCode == 200) {
        ...
        final trackingInfo = DeliveryTrackingInfo.fromJson(
          data['data'] ?? data,
        );
        ...
        debugPrint(
          '📍 Position mise à jour: ${trackingInfo.courierPosition?.latLng}',
        );
