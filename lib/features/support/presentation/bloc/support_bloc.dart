...
class SupportBloc extends Bloc<SupportEvent, SupportState> with SafeEmitMixin {
  ...
  SupportBloc(this._repository) : super(const SupportState()) {
    ...
    on<LoadComplaintDetails>(
      _onLoadComplaintDetails,
  ...
  Future<void> _onLoadContactInfo(
    ...
    try {
      ...
      emit(
        state.copyWith(status: SupportStatus.success, contactInfo: contactInfo),
      );
    } catch (e) {
      emit(
        state.copyWith(
  ...
  Future<void> _onLoadFaqs(LoadFaqs event, Emitter<SupportState> emit) async {
    try {
  ...
  Future<void> _onViewFaq(ViewFaq event, Emitter<SupportState> emit) async {
    try {
      await _repository.viewFaq(event.faqId);
    } catch (e) {
      debugPrint('ViewFaq error: $e');
    }
  }
  ...
  Future<void> _onLoadChats(LoadChats event, Emitter<SupportState> emit) async {
    try {
  ...
  Future<void> _onLoadChatMessages(
    ...
    try {
      ...
      emit(state.copyWith(chatMessages: messages, chatLoading: false));
    } catch (e) {
      emit(
        state.copyWith(
          chatLoading: false,
  ...
  Future<void> _onSendChatMessage(
    ...
    try {
      ...
      final message = await _repository.sendChatMessage(
        event.chatId,
        event.message,
      );
      final updatedMessages = [...state.chatMessages, message];
      emit(
        state.copyWith(chatMessages: updatedMessages, sendingMessage: false),
      );
    } catch (e) {
      emit(
        state.copyWith(
  ...
  Future<void> _onCloseChat(CloseChat event, Emitter<SupportState> emit) async {
    try {
      ...
      if (isClosed) return;
      emit(
        state.copyWith(errorMessage: 'Erreur lors de la fermeture du chat: $e'),
      );
    }
  }
  ...
  Future<void> _onLoadComplaintDetails(
    ...
    try {
      ...
      final messages = await _repository.getComplaintMessages(
        event.complaintId,
      );
      emit(
        state.copyWith(
  ...
  Future<void> _onAddComplaintMessage(
    ...
    try {
      emit(state.copyWith(sendingMessage: true));
      final message = await _repository.addComplaintMessage(
        event.complaintId,
        event.message,
      );
